...
class RegisterActivity : AppCompatActivity() {
    ...
    private fun actionDone(it: MenuItem) {
        ...
        if (txt_phone.text.toString().isEmpty()) {
            Toast.makeText(applicationContext, "No Handphone tidak valid", Toast.LENGTH_LONG).show()
            return
        }
        val radioButton: RadioButton = findViewById(rg_gender.checkedRadioButtonId)
        ...
        req = setRequest(
            "auth.register",
            mapOf(
                "full_name" to txt_name.text.toString(),
                "email" to txt_email.text.toString(),
                "phone" to txt_phone.text.toString(),
                "gender" to radioButton.tag.toString()
            )
        )
